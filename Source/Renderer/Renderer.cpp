...
RenderableResourceId Renderer::CreateRenderable(const RenderableCreateInfo& createModelResourceInfo)
{
	...
	model->FrameResources = CreateModelFrameResources((u32)_swapchainImages.size(), *model);
...
void Renderer::SetMaterial(const RenderableResourceId& renderableResId, const Material& newMat)
{
	...
	WriteDescriptorSets((u32)count, descriptorSets, 
		modelBuffers,
		_lightBuffers,
...
void Renderer::InitVulkan()
{
	...
	_pbrDescriptorSetLayout = CreateDescriptorSetLayout(_device);


	// Create Pipeline Layout  -  Used to pass uniforms to shaders at runtime
	auto CreatePipelineLayout = [](VkDevice device, VkDescriptorSetLayout descriptorSetLayout)
	{
	...
	_pipelineLayout = CreatePipelineLayout(_device, _pbrDescriptorSetLayout);
...
void Renderer::CreateSwapchainAndDependents(int width, int height)
{
	...
	_pipeline = CreatePbrGraphicsPipeline(_shaderDir, _pipelineLayout, _msaaSamples, _renderPass, _device,
		_swapchainExtent);
...
std::vector<ModelResourceFrame> Renderer::CreateModelFrameResources(u32 numImagesInFlight, 
	const Renderable& renderable) const
{
	...
	std::vector<VkDescriptorSet> descriptorSets = CreateDescriptorSets(
		(uint32_t)numImagesInFlight,
		_pbrDescriptorSetLayout, _descriptorPool,
		modelBuffers,
		_lightBuffers,
		*_textures[basecolorMapId],
		*_textures[normalMapId],
...
VkDescriptorSetLayout Renderer::CreateDescriptorSetLayout(VkDevice device)
{
	...
	VkDescriptorSetLayoutBinding pbrUboLayoutBinding = {};
	{
		...
		pbrUboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
	...
	{
		skyboxMapLayoutBinding.binding = 7; // correlates to shader
